...
const register = async () => {
    ...
    try {
        ...
        await set(ref(db, 'users/' + cred.user.uid), { categories: ['General'], tools: [], history: [] });
...
window.toggleLoan = (i) => {
    if (!tools[i].inUse) {
        const p = prompt("Nombre de quien retira:");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-add-category').onclick = () => {
        ...
        if (val) { categories.push(val); sync(); document.getElementById('new-category-name').value = ''; }
    };
    ...
    document.getElementById('btn-add-budget-item').onclick = () => {
        ...
        if (desc && price) {
            ...
            calculateTotals();
            document.getElementById('budget-list-display').innerHTML = budgetItems.map(it => `<div>${it.desc} - $${it.price}</div>`).join('');
        }
    };

    document.getElementById('btn-finish-work').onclick = () => {
        const t = calculateTotals();
        budgetHistory.push({ id: Date.now(), date: new Date().toISOString(), client: document.getElementById('client-name').value || "Gral", total: t });
        budgetItems = []; document.getElementById('budget-list-display').innerHTML = '';
        sync();
    };

    document.getElementById('btn-delete-history').onclick = () => {
    ...
    document.getElementById('btn-generate-pdf').onclick = () => {
        ...
        const doc = new jsPDF();
        doc.text("Presupuesto Taller", 10, 10);
